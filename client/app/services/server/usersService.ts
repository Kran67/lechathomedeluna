 ...
 * @returns { Promise<any> } Un object contenant l'utilisateur ou un object contenant une erreur
 */
export const update = cache(async (token: string | undefined, id: string, name: string, lastName: string, social_number: string, phone: string, address: string, city: string, roles: string, blacklisted: boolean, referrer_id: string | null, capacity: string) => {
    try {
        const res: Response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/users/${id}`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}`, },
            body: JSON.stringify({ name, lastName, social_number, phone, address, city, roles, blacklisted, referrer_id, capacity }),
        });
