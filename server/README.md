# Lechathomedeluna Backend (API Express + SQLite)

Backend minimaliste pour l’application Lechathomedeluna. Il expose une API REST (Express 5) documentée via OpenAPI, utilise SQLite pour le stockage des données, gère l’authentification par JWT et propose des fonctionnalités autour des chats, des utilisateurs, ainsi que l’upload et la suppression d’images.
...
- Lister, créer, modifier, supprimer des chats et consulter leur détail.
...
## Base de données & données de démo
- SGBD: SQLite, fichier: data/lechathomedeluna.sqlite3
- À l’initialisation, le schéma est créé automatiquement. Si aucun chat n’existe, un seed est effectué depuis data/cats.json (si présent).
- Le schéma inclut: users, properties, cat_pictures, cat_vaccines.
- Les slugs des chats sont générés automatiquement et uniques.

Sauvegarde: le fichier SQLite (data/lechathomedeluna.sqlite3) est persistant. Pour repartir de zéro, stoppez le serveur et supprimez ce fichier (et relancez pour recréer/seed).
...
- Rôles supportés: client, owner, admin.
  ...
  - D’autres nécessitent un rôle spécifique, par ex. owner ou admin pour créer/mettre à jour/supprimer des chats, ou pour les uploads.
...
Base: /api
- GET /api/properties: liste des chats
- GET /api/properties/:id: détail d’un chat
...
- GET /api/properties/:id/ratings: lister les notes d’un chat
...
- Upload (multipart/form-data): champ file obligatoire. Champs optionnels: purpose (cat-picture | user-picture | other), cat_id (validation d’existence).
...
- Repartir de zéro: stoppez le serveur, supprimez data/lechathomedeluna.sqlite3, relancez (recréation du schéma et seed depuis data/cats.json si disponible).
